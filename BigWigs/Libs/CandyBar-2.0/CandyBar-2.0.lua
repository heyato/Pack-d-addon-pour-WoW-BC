...
local vmajor, vminor = "CandyBar-2.0", "$Revision: 45988 $"
...
function CandyBar:SetGradient(name, c1, c2, ...)
	...
	elseif type(c1) == "number" then
		...
		local num = (n - 1) / 3 -- How many RGB pairs we have (in addition to the first one)
		local gtId = 2
		for i = 1, num, 3 do
			local r, g, b = select(i + 1, ...)
			if r and g and b then
				gtable[gtId] = new()
				gtable[gtId][1] = r
				gtable[gtId][2] = g
				gtable[gtId][3] = b
				gradientid = string.format("%s%d%d%d", gradientid, r, g, b)
				gtId = gtId + 1
			else
				break
			end
		...
		elseif v.running then
			...
			if v.endtime <= t then
				...
				if c.completion then
					c.completion(getArgs(c, "completion"))
				end
				this.owner:Stop(i)
			else
				this.owner:Update(i)
...
AceLibrary:Register(CandyBar, vmajor, vminor, activate)
