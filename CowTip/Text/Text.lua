...
local VERSION = tonumber(("$Revision: 66703 $"):match("%d+"))
...
if CowTip.revision < VERSION then
	...
	CowTip.date = ("$Date: 2008-03-28 11:39:23 -0400 (Fri, 28 Mar 2008) $"):match("%d%d%d%d%-%d%d%-%d%d")
end
...
function CowTip_Text:OnInitialize()
	...
	CowTip:SetDatabaseNamespaceDefaults("Text", "profile", {
		...
		rights = {
			...
			"[if Guild = 'player':Guild then Green][Guild:Angle]", -- [3]
			"[GuildRank]", -- [4]
